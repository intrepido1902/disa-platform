...
export const QuoteModal = ({ isOpen, onClose }: QuoteModalProps) => {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      // Continúa aunque Supabase falle — WA es lo crítico
    }
    setLoading(false);

    const msg = encodeURIComponent(
      `Hola DISA 👋 Soy ${form.nombre.trim()} y me gustaría cotizar ${form.producto} para un proyecto ${
        form.tipo === "b2b" ? "corporativo/empresa" : "residencial/hogar"
      }. Mi email es ${form.email.trim()} y mi WhatsApp es ${form.whatsapp}. ¿Pueden asesorarme?`
    );
      ...
      {isOpen && (
        <>
          ...
          <motion.div
            ...
            className="fixed inset-x-4 top-1/2 -translate-y-1/2 md:inset-auto md:left-1/2 md:-translate-x-1/2 md:w-full md:max-w-xl z-[201] bg-white overflow-hidden"
